...
def main():
    ...
    add_paragraph(
        ...
        "结果表明，单次随机路径和均值指标可能掩盖尾部拥堵风险，因此安防决策应关注超过阈值的概率和分位数；"
    ...
    for i, item in enumerate(toc_items):
        p = add_paragraph(doc, item, first_line=False)
    ...
    add_paragraph(
        ...
        "即积分约束条件。系统模拟时，作为连续时间非齐次泊松过程的离散时间近似，本项目采用分钟级离散化的非齐次泊松增量模拟，"
        "即在每一分钟生成 A(t) ~ Poisson(λ(t)Δt)（当时间步长 Δt = 1 分钟时，等价于 Poisson(λ(t))）。"
        "这在保证独立增量性质的同时，也极大提升了仿真计算效率。而 Lewis & Shedler (1979) 提出的接受-拒绝减薄算法（Thinning Algorithm）"
        "则是连续时间非齐次泊松过程的经典精确生成方法，本项目的分钟级离散化是一种实用且高效的数值近似。",
    )
    ...
    add_paragraph(
        doc,
        "图 2 以热力图形式展示了通道数（32～38 个）与到达集中度（0.80～1.80）"
        "两个参数正交扫描时的拥堵风险概率。"
        "结果表明，系统拥堵风险并非随参数线性变化，而是存在明显的容量临界区。"
        "当通道数不足（如 32 或 33 个）时，由于基础容量严重匮乏，错峰入场难以单独消除拥堵，风险高达 100.0%；"
        "当通道数接近临界容量（如 36 个）时，系统处于高度敏感状态，"
        "到达集中度的小幅上升（如从 1.00 升至 1.20 再到 1.40）会导致拥堵概率从 0.0% 跃升至 26.7%（热力图中向上取整显示为 27%），再急剧升至 91.7%（热力图中显示为 92%），"
        "这极为直观地展示了到达集中度对拥堵排队系统的决定性影响；"
        "而当通道数达到足够冗余水平（如 38 个）时，即使高集中度到达也能被系统吸收，"
        "在本次蒙特卡洛模拟中各集中度下均未观察到超过阈值的拥堵事件。"
        "这一规律揭示了“错峰入场”与“通道冗余”结合使用的最优设计思路："
        "最优策略并不是单纯增加通道或单纯错峰，而是在保证基本服务容量的前提下，通过错峰入场降低峰值冲击。",
    )
