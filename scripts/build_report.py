...
def main():
    ...
    add_heading(doc, "3.3 高阶叙事性模拟的 Web 应用设计", 2)
    add_paragraph(
        doc,
        "本项目的交互作品不是单纯展示静态图表，而是按照“带步骤的引导式分析报告”进行设计。网页将大型活动安防过程拆分为五个连续情节："
        "第一幕展示观众并非匀速到达，而是在开场前形成随机波峰；第二幕展示当瞬时到达人数超过安检服务能力时，队列如何逐步积累并形成拥堵风险；"
        "第三幕通过贝叶斯后验概率解释为什么在低基率场景下，报警并不必然意味着真实风险；第四幕把场内巡逻抽象为二维随机游走，"
        "展示热点偏向如何缩短异常点的首达时间；第五幕综合比较错峰入场、增开通道、智能巡逻和组合优化方案，形成从风险识别到策略选择的完整闭环。",
    )
    add_paragraph(
        doc,
        "在表现形式上，网页同时结合文字、图表、动态模拟和关键数据高亮。左侧叙事面板用文字解释当前步骤对应的概率模型和现实含义；"
        "右侧图表用 Canvas 动态绘制入场事件流、队列演化、巡逻路径和方案对比；顶部关键指标卡实时显示最大队列、拥堵风险、平均等待时间、"
        "报警可信度、巡逻发现中位数和误报核查成本。用户调整到达集中度、安检通道数、服务效率、拥堵阈值、误报率和巡逻偏向后，"
        "系统会立即重新进行随机模拟和蒙特卡洛估计，使用户能够直观看到“参数改变 → 风险响应 → 策略解释”的过程。",
    )
    add_paragraph(
        doc,
        "因此，该 Web 应用承担了类似“新冠传播模拟”或“赌徒破产模拟”的叙事功能：它不只是给出一个最终答案，而是让用户沿着故事线逐步理解随机过程如何影响现实决策。"
        "通过这种设计，概率模型从抽象公式转化为可观察、可调节、可解释的安防管理情境，更符合高阶叙事性模拟对完整故事、动态过程和数据解释的要求。",
    )
